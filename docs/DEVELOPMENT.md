...
- [x] Worker 共享 lib 使用固定基线目录、嵌入 JAR 内容比较、临时文件和原子替换；客户端同步后再
  建立 IPC，避免缺失或损坏的依赖被 Worker 使用。
...
- [x] 客户端注册表完成后按当前语言导入全部物品 ID、显示名称和静态 LORE/翻译描述；启动扫描不调用动态 Tooltip，物品目录与手册 FTS 分表保存。
- [x] 物品目录写入使用 Worker 单事务批量 UPSERT；大批量记录通过独立 I/O 线程的原子 JSONL 载荷传递，IPC 不发送超大 JSON 数组；首次、语言切换和增量更新都不复制或替换整个数据库；相同注册表/语言指纹的后续启动复用 `runtime/knowledge/cache/`，不重复执行全量静态捕获。
- [x] Jade 确认物品后，AI 与仅搜索模式先读取 `item_catalog`，再继续手册搜索，并把静态物品简介作为独立事实上下文。
- [x] 静态简介缺失时，Worker 通过 `runtime_item_context` 按需请求最多 3 个已确认物品的当前世界 Tooltip；运行时 Tooltip 不落库、不写会话或日志。
- [~] 在大型整合包中确认静态目录批量导入耗时、语言切换、动态 Tooltip 内容质量和真实客户端回归。
