...
`worker-baseline-3`，只适用于本仓库当前的 NeoForge 1.21.1 客户端适配层；它不是
...
`java.class.path`。SLF4J API 和 Gson 都从 Worker 专用资源提取，不能从 Forge/NeoForge 游戏类
加载器取用，也不能把它们放入普通 Jar-in-Jar 目录。这样游戏实例中残留的旧版
