...
| Worker 专用依赖 | SLF4J API **2.0.9** 与 Gson 放在 `META-INF/modpedia-worker/` |
...
`~/.modpedia/worker/lib/worker-baseline-3/`，而日志、IPC 状态和临时 payload 仍留在当前实例的
...
`~/.modpedia/worker/lib/worker-baseline-3/`，也不属于整合包，不要复制或打包；同一 Worker 基线的
