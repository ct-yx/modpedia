...
7. `search_tasks` 返回的 `task_static_definition` 是任务定义，`task_runtime_progress` 是玩家实时进度，`wiki_reference` 是说明文档。未同步时必须明确说明，不得用 Wiki 猜测当前进度；NEXT 有多个候选时列出候选，不伪造唯一主线。随机奖励的 `candidates` 只是候选列表，`is_random=true` 时不得写成确定奖励。
